...
class PGPKeyAlgSpec
  ...
  test("PGPKeyAlg should load an arbitrary armored PGP private key") {
    ...
    forAllF(genWeakKeyPair[IO](blocker), arbitrary[Array[Char]]) { (kp, passphrase) =>
  ...
  test("PGPKeyAlg should load an exported secret key into a key ring") {
    ...
    for {
      ...
      count <- Stream.fromBlockingIterator[IO](blocker, output.iterator().asScala)
        .flatMap(ring => Stream.fromBlockingIterator[IO](blocker, ring.iterator().asScala))
        .map(_ => 1)
        .compile
        .foldMonoid
    } yield {
