...
class CryptoAlgSpec
  ...
  private def genNelResource[F[_], A](implicit
      A: Arbitrary[Resource[F, A]]
  ): Gen[Resource[F, NonEmptyList[A]]] =
    for {
  ...
  private implicit def arbNelResource[F[_], A](implicit
      A: Arbitrary[Resource[F, A]]
  ): Arbitrary[Resource[F, NonEmptyList[A]]] = Arbitrary(genNelResource[F, A])

  test(
    "CryptoAlg should round trip the plaintext using one or more key pairs"
  ) {
    val crypto = resource()
    ...
    forAllF {
      (
          keyPairsR: Resource[IO, NonEmptyList[PGPKeyPair]],
          ...
          LoggerFactory[IO]
            ...
            .flatMap { implicit l =>
              for {
                ...
                _ <- Logger[IO]
                  .trace(
                    s"encrypting with keys ${allRecipients.map(_.getKeyID)}"
                  )
                  .toResource
                encryptedBytes <-
                  bytes
                    .through(
                      crypto.encrypt(
                        allRecipients,
                        EncryptionConfig().withChunkSize(encryptionChunkSize)
                      )
                    )
                ...
                decryptedBytes <-
                  privateKeys.traverse { privateKey =>
                    Logger[IO]
                      .trace(s"decrypting with key id ${privateKey.getKeyID}")
                      .toResource >>
                      Stream
                        .emits(encryptedBytes)
                        .through(
                          crypto.decrypt(privateKey, decryptionChunkSize)
                        )
                        .compile
                        .resource
                        .toList
  ...
  test("CryptoAlg should maintain chunk size throughout pipeline") {
    ...
    case class Inputs(
        keyPairR: Resource[IO, PGPKeyPair],
        encryptionChunkSize: ChunkSize,
    ...
    forAllNoShrinkF(genChunkSizeTestInputs) {
      case Inputs(keyPairR, encryptionChunkSize, bytes) =>
        val testResource =
          for {
            keyPair <- keyPairR
            chunkSizes <- bytes
  ...
  test(
    "CryptoAlg should round trip the plaintext using a key ring collection"
  ) {
    val crypto = resource()
      ...
      case Inputs(
        ...
        val testResource =
          for {
            ...
            pub <- keysIn[IO](collection)
              .map(_.getPublicKey)
              .find(_.isEncryptionKey)
              .compile
              .resource
              .lastOrError
            roundTrip <- Stream
              .emits(materializedBytes)
              .through(
                crypto.encrypt(
                  EncryptionConfig().withChunkSize(encryptionChunkSize),
                  pub
              ...
              .through(crypto.armor(encryptionChunkSize))
              .through(
                crypto.decrypt(collection, passphrase, decryptionChunkSize)
              )
              .compile
              .resource
              .toList
      ...
      (
        ...
        val testResource =
          for {
            kp <- keyPairR
            ring <- Resource
              .eval(pgpKeyRingGenerator[IO](keyRingId, kp, passphrase))
              .map(_.generateSecretKeyRing())
            roundTrip <- Stream
              .emits(materializedBytes)
              .through(
                crypto.encrypt(
  ...
  private implicit def prettyArrayChar: Array[Char] => Pretty = arr =>
    Pretty { _ =>
