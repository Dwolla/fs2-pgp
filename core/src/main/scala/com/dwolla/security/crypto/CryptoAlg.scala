...
trait CryptoAlg[F[_]] extends Encrypt[F] with Decrypt[F] with Armor[F]
...
object CryptoAlg {
  ...
  def apply[F[_]: Async: LoggerFactory](implicit
      ev: BouncyCastleResource
  ): F[CryptoAlg[F]] =
    for {
      d <- LoggerFactory[F]
        .create(LoggerName("com.dwolla.security.crypto.Decrypt"))
        .map(implicit l => Decrypt[F])
      e <- LoggerFactory[F]
        .create(LoggerName("com.dwolla.security.crypto.Encrypt"))
        .map(implicit l => Encrypt[F])
      a = Armor[F]
