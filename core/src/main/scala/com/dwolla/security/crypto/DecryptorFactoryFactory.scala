...
trait DecryptorFactoryFactory[F[_], A] {
  def publicKeyDataDecryptorFactory(input: A, keyId: Long, passphrase: Array[Char]): F[PublicKeyDataDecryptorFactory]
}

object DecryptorFactoryFactory {
  def apply[F[_], A](implicit DFF: DecryptorFactoryFactory[F, A]): DecryptorFactoryFactory[F, A] = DFF

  implicit def PGPSecretKeyRingCollectionInstance[F[_] : Sync]: DecryptorFactoryFactory[F, PGPSecretKeyRingCollection] =
    reflectiveDecryptorFactoryFactoryInstance(_, _, _)

  implicit def PGPSecretKeyRingInstance[F[_] : Sync]: DecryptorFactoryFactory[F, PGPSecretKeyRing] =
    reflectiveDecryptorFactoryFactoryInstance(_, _, _)
  ...
  implicit def PGPPrivateKeyInstance[F[_] : Sync]: DecryptorFactoryFactory[F, PGPPrivateKey] =
